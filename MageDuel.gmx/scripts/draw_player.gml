...
// Get data
    ...
    for(i=0;i<global.playerTotal;i++){
        var sp,spindex,col,activeSpell;
...
for(i=1;i<blocks+1;i++){
    var x1 = playerX-(sprite_width/4) + (sprite_width/2)*(i/4)-(size/2);
    var x2 = playerX-(sprite_width/4) + (sprite_width/2)*(i/4)+(size/2);
...
var x1 = playerX-(sprite_width/2) + (sprite_width-length)/2;
...
var x1 = playerX-(sprite_width/2) + (sprite_width-textLength)/2;
